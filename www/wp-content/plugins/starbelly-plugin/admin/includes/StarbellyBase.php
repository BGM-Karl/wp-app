<?php
if(!class_exists("StarbellyBase")) {
	class StarbellyBase {
    	public $key = "33557E999686BCE7";
    	private $product_id = "20";
    	private $product_base = "starbelly";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_starbelly_fupc', function(){
						update_option('_site_transient_update_plugins','');
						update_option('_site_transient_update_themes','');
						set_site_transient('update_themes', null);
                        delete_transient($this->product_base."_up");
						wp_redirect(  admin_url( 'themes.php?theme=starbelly' ) );
						exit;
					});
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=starbelly_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
                ...
                if (!is_wp_error($response)) {
                    ...
                    if ( is_object( $responseJson ) && ! empty( $responseJson->status ) && ! empty( $responseJson->data->new_version ) ) {
                        ...
                        unset( $responseJson->data->IsStoppedUpdate );
												delete_metadata( 'user', null, 'starbelly_dismissed_notice', null, true );
