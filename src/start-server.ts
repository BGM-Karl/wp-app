...
const requestBodyToBytes = async (req: Request<{}, any, any, ParsedQs, Record<string, any>>): Promise<Uint8Array> =>
...
function shouldCompress(_: any, res: { getHeader: (arg0: string) => any; }) {
...
export async function startServer(
	...
	app.use('/', async (req, res) => {
		try {
			...
			if (isWebContainer() && options.absoluteUrl) {
	...
	if (!url) {
		throw new Error('No landing page or absolute URL provided');
	}
